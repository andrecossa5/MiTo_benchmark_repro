...
# Explore ...
# df[['n_cells', 'n_vars', 'median_n_vars_per_cell', 'corr', 'transitions_vs_transversions_ratio', 'mean_CI']].describe().T
# df.sort_values('n_vars').tail(1).T.to_dict()
...
df_selected = (
    df#.query('corr>.25 and n_cells>1000 and n_vars>10') 
...
df_selected = df_selected.groupby('cat').apply(lambda x: x.sort_values('corr', ascending=False).head(3)).reset_index(drop=True)
df_selected

# Write out
# L = []
# path_remote = '/data/cossa2_dare/AML_MITO/data/AFMs'
