...
path_results = os.path.join(path_main, f'results/AML/final_trees/sAML1/tuning93742a155d')
...
plot_tree(tree, ax=ax, feature_internal_nodes='support', internal_node_subset=mut_nodes,
          cmap_internal_nodes=('Spectral_r',.5,.7), internal_node_kwargs={'markersize':5})
...
ax = axs[1]
plot_tree(tree, features=['tumor_tme_class_new', 'MT_clone'], ax=ax, categorical_cmaps=cmaps, colorstrip_width=10)
...
plot_tree(tree, ax=axs[0], 
    ...
    orient='down',
    features=['MT_clone']+mutation_order, layer='raw',
    categorical_cmaps=cmaps,
    feature_label_size=1, feature_label_offset=2,
)
...
plot_tree(tree, ax=axs[1],
    colorstrip_spacing=.000001, colorstrip_width=2,
    orient='down',
    features=['MT_clone']+mutation_order, layer='transformed',
    feature_label_size=1, feature_label_offset=2,
    categorical_cmaps=cmaps,
