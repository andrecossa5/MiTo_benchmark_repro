...
for sample in samples:
    afm = sc.read(os.path.join(path_afms, sample, f'afm_unfiltered.h5ad'))
...
fig.tight_layout()
fig.savefig(os.path.join(path_figures, 'Fig_2a.pdf'))
...
for i,sample in enumerate(samples):
    cov = pd.read_csv(os.path.join(path_afms, sample, 'coverage.txt.gz'), header=None)
...
fig.tight_layout()
fig.savefig(os.path.join(path_figures, 'Fig_2c.pdf'))
...
fig.tight_layout()
fig.savefig(os.path.join(path_figures, 'Fig_2c_legend.pdf'))


##


# 2. nf-MiTo example (MDA_PT sample) --------------------------------------- # 

# Choose example nf-MiTo ourtput / filtered MT-SNVs space
sample = 'MDA_PT'
job_id = 'a4a3ca88dd'
path_data = os.path.join(path_main, 'data', 'lineage_inference', 'UPMGA', sample, job_id)

# Load: afm, tree
afm_unfiltered = sc.read(os.path.join(path_afms, sample, 'afm_unfiltered.h5ad'))
...
fig.tight_layout()
fig.savefig(os.path.join(path_figures, 'Fig_2def.pdf'))
...
fig.savefig(os.path.join(path_figures, 'Fig_2g.pdf'))
...
fig.tight_layout()
fig.savefig(os.path.join(path_figures, 'Fig_2h.pdf'))
...
fig.tight_layout()
fig.savefig(os.path.join(path_figures, 'Fig_2i.pdf'))
...
fig.tight_layout()
fig.savefig(os.path.join(path_figures, 'Fig_2lm.pdf'))
...
tree.cell_meta['MiTo clone'] = tree.cell_meta['MiTo clone'].astype('str')
tree.cell_meta.loc[tree.cell_meta['MiTo clone'] == 'nan', 'MiTo clone'] = 'unassigned'
cmaps = { 
    'GBC' : clones_colors,
    'MiTo clone' : plu.create_palette(
        tree.cell_meta, 'MiTo clone', col_list=sc.pl.palettes.default_102, add_na=True
    )
}
...
fig.savefig(os.path.join(path_figures, 'Fig_2n.pdf'))
