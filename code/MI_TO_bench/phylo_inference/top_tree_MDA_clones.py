...
path_afm_raw = os.path.join(path_main, f'data/MI_TO_bench/AFMs/mito_preprocessing/{sample}/afm.h5ad')
...
# Embeddings
gbcs = ['CACGATCCGTCGGCCCAG', 'CATACAGGTGACCGTGGC', 'CCTTTAGCAAGCCCGAAA']
gbc_colors = create_palette(afm.obs, 'GBC', sc.pl.palettes.vega_10)
...
for i,x in enumerate(gbcs):
    ...
    s = f'GBC=="{x}"'
    draw_embeddings(afm.obs, cat='GBC', query=s, s=10, ax=axs[0,i],legend_kwargs={'colors':gbc_colors}, 
...
draw_embeddings(afm.obs, cat='GBC', s=50, ax=ax, legend_kwargs={'colors':gbc_colors}, axes_kwargs={'legend':False})
...
with open(os.path.join(path_results, 'annotated_tree.pickle'), 'rb') as f:
    tree = pickle.load(f)
# write_newick(tree, os.path.join(path_results, 'annotated_tree.newick'))
...
cmaps = {
    'GBC' : create_palette(tree.cell_meta, 'GBC', sc.pl.palettes.vega_10),
